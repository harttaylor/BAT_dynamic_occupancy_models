################################################################################
# 03_visualize_results.R — Dynamic Occupancy Figures + Text Summaries
#
# Based on original working BC script, updated for BC or BCAK mode.
# Set MODE below. The only difference is BCAK has fewer covariate effects
# (no dist_harvest) and JAGS names scalar params without [1] index.
#
# Per-species outputs:
...
#   6. All-species lambda comparison
...
extract_results <- function(species, input_dir, data_dir) {
  ...
  # Trend summary
  trend <- data.frame(
    species = species, nsite = d$nsite, nyear = nyear,
  ...
  # Covariate effects — BC vs BCAK differ
  if (HAS_HARVEST) {
    coef <- data.frame(
      species = species,
      parameter = c("psi: Elevation", "psi: Water Nearby",
                    "gamma: Dist to Road", "gamma: Dist to Harvest",
                    "phi: Dist to Road", "phi: Dist to Harvest",
                    "p: Clutter", "p: Temperature", "p: Julian"),
      category = c(rep("Initial Occupancy", 2), rep("Colonization", 2),
                   rep("Persistence", 2), rep("Detection", 3)),
      estimate = c(summ["beta.psi[2]","mean"], summ["beta.psi[3]","mean"],
      ...
      lcl = c(summ["beta.psi[2]","2.5%"], summ["beta.psi[3]","2.5%"],
      ...
      ucl = c(summ["beta.psi[2]","97.5%"], summ["beta.psi[3]","97.5%"],
    ...
    # BCAK: scalar beta.gamma / beta.phi (no [1] index)
    coef <- data.frame(
      species = species,
      parameter = c("psi: Elevation", "psi: Water Nearby",
                    "gamma: Dist to Road",
                    "phi: Dist to Road",
                    "p: Clutter", "p: Temperature", "p: Julian"),
      category = c(rep("Initial Occupancy", 2), "Colonization",
                   "Persistence", rep("Detection", 3)),
      estimate = c(summ["beta.psi[2]","mean"], summ["beta.psi[3]","mean"],
                   summ["beta.gamma","mean"],
                   summ["beta.phi","mean"],
                   summ["beta.p[2]","mean"], summ["beta.p[3]","mean"], summ["beta.p[4]","mean"]),
      lcl = c(summ["beta.psi[2]","2.5%"], summ["beta.psi[3]","2.5%"],
              summ["beta.gamma","2.5%"],
...
cat_colors <- c("Initial Occupancy" = "#1B9E77", "Colonization" = "#D95F02",
                "Persistence" = "#7570B3", "Detection" = "#666666")
...
cat("Processing:", paste(species_list, collapse = ", "), "\n\n")
...
for (sp in species_list) {
  ...
  cat(" OK (lambda =", round(res$trend$lambda_mean, 2), ")\n")
}
...
param_order <- unique(coef_df$parameter)

cat("\nLoaded", length(all_occ), "species\n\n")

# ============================================================================
# PER-SPECIES FIGURES + TEXT
# ============================================================================

cat("Creating figures...\n")

for (sp in names(all_occ)) {
  ...
  p1 <- ggplot(sp_occ, aes(x = year, y = psi)) +
    ...
    labs(title = paste0(sp, " \u2014 Occupancy Trend (", MODE_LABEL, ", ", year_range, ")"),
  ...
  p2 <- ggplot(sp_dyn) +
    ...
    labs(title = paste0(sp, " \u2014 Dynamics (", MODE_LABEL, ", ",
  ...
  p3 <- ggplot(sp_coef, aes(x = estimate, y = parameter, color = category)) +
    ...
    labs(title = paste0(sp, " \u2014 Covariate Effects (", MODE_LABEL, ", ", year_range, ")"),
  ...
  trend_word <- if (sp_trend$lambda_ucl < 1) "DECLINE" else
    if (sp_trend$lambda_lcl > 1) "INCREASE" else "STABLE (no clear trend)"
  
  txt <- c(
    paste0("RESULTS: ", sp, " (", MODE_LABEL, ")"),
    paste0(rep("=", 50)), "",
  ...
  if (nrow(sig_covs) == 0) { txt <- c(txt, "  None") } else {
...
if (nrow(trend_df) > 1) {
  ...
  p_panel <- ggplot(occ_df, aes(x = year, y = psi)) +
    ...
    labs(title = paste0("Occupancy Trends (", MODE_LABEL, ", ", yr_label, ")"),
         x = "Year", y = "Occupancy (\u03c8)") +
  ...
  p_coef_all <- ggplot(coef_df, aes(x = estimate, y = species, color = significant)) +
    ...
    labs(title = paste0("Covariate Effects (", MODE_LABEL, ", ", yr_label, ")"),
  ...
  p_dyn_panel <- ggplot(dyn_long, aes(x = year_to, y = mean, color = rate, fill = rate)) +
    ...
    labs(title = paste0("Dynamics (", MODE_LABEL, ", ",
  ...
  summary_table <- trend_df %>%
    mutate(
      ...
      trend = case_when(lambda_ucl < 1 ~ "Decline",
                        lambda_lcl > 1 ~ "Increase", TRUE ~ "Stable")
    ) %>%
